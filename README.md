...
**README用のSVGバナーをブラウザだけで生成する。外部フォント不要、8テーマ+7種の柄+3書体、ライブプレビュー付き。**
...
テーマは藍・深緑・夕暮れ・墨・桜・海・琥珀・葡萄の8種、柄は水玉・方眼・斜線・波・青海波・十字・山形から選べる。書体はゴシック・明朝・等幅の3系統、背景はグラデーションとべた塗りを切り替えられる。いずれもシステムフォントスタック指定なので、外部フォントの読み込みがなく、どの環境でも即座に描画される。タイトルの長さに応じて文字サイズが自動で縮む。

操作画面はライト・ダークの両方に対応し、切替はOS追従・ライト固定・ダーク固定の三択で選べる。最後に作った設定はブラウザに残るので、次に開いたときは前回の続きから始められる。
...
1. [公開ページ](https://miruky.github.io/nobori/)でタイトル・サブタイトル・テーマ・柄・書体・背景・サイズ・配置を選ぶ。配色に迷ったら「おまかせ」で見た目の4項目を振り直せる
2. プレビューを確認して「SVGをダウンロード」(SVGソースのコピーも可)。SNS等で画像が必要なら「PNGをダウンロード」で2倍解像度のPNGも書き出せる
3. リポジトリに置いてREADMEから参照する。「Markdownをコピー」を押せば `![タイトル](banner.svg)` の貼り付け用記法がそのままクリップボードに入る

主な操作にはキーボードショートカットを割り当てている。<kbd>D</kbd> でSVG保存、<kbd>C</kbd> でソースのコピー、<kbd>R</kbd> でおまかせ、<kbd>T</kbd> でテーマ切替。入力欄にカーソルがある間は誤爆しないよう無効になる。
...
設定はURLのハッシュ(`#t=...&th=...`)に保存される。「共有リンクをコピー」でその時の見た目をそのまま渡せ、開いた相手は同じバナーから調整を始められる。手で書き換えられた未知のテーマ・柄・書体・背景は読み込み時に既定へ落ちる。

読み込み時は、URLハッシュ(共有された設定)を最優先し、無ければ前回ブラウザに残した設定、それも無ければ初期値の順で復元する。

## プロジェクト構成

- `src/`
  - `banner.ts` — テーマ・書体・背景の定義とSVG生成(純関数)
  - `banner.test.ts` — エスケープ・寸法・全テーマ×全柄・書体・背景のテスト
  - `share.ts` + `share.test.ts` — 設定とURLハッシュの相互変換(純関数)
  - `theme.ts` + `theme.test.ts` — UIテーマ(明暗)の解決(純関数)
  - `presets.ts` + `presets.test.ts` — おまかせ生成とMarkdownスニペット(純関数)
  - `main.ts` — フォームとプレビューの結線、ショートカット
...
**純粋な生成と副作用を分ける。** `banner.ts` と `share.ts` と `theme.ts` はDOMにもlocalStorageにも触れない純関数だけで構成し、テストはそれらに対して書く。クリップボード・ファイル保存・ストレージといった副作用は `main.ts` に閉じ込め、UIの差し替えがあっても生成ロジックを汚さないようにしている。
