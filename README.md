...
**README用のSVGバナーをブラウザだけで生成する。外部フォント不要、8テーマ+7種の柄、ライブプレビュー付き。**
...
テーマは藍・深緑・夕暮れ・墨・桜・海・琥珀・葡萄の8種、柄は水玉・方眼・斜線・波・青海波・十字・山形から選べる。書体はシステムフォントスタック指定なので、外部フォントの読み込みがなく、どの環境でも即座に描画される。タイトルの長さに応じて文字サイズが自動で縮む。
